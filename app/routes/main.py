...
from app.services.auth import load_users, create_user, BASE_DIR # load_users shim used for login selection
from app.services.data_manager import (
    get_dashboard_stats, sync_manifest, log_receipt, load_config, MANIFEST_FILE,
    get_scan_count_today, get_file_age, check_history
)
...
def index():
    ...
    if 'user' not in session:
        logger.info(f"[Index] No user in session. Session: {dict(session)}")
        return redirect(url_for('main.login'))
    
    logger.info(f"[Index] User found: {session.get('user')}. Session: {dict(session)}")
        
    stats = get_dashboard_stats()
    scans = get_scan_count_today()
...
def setup_wizard():
    ...
    if request.method == 'POST':
        ...
        pin = request.form.get('first_pin', '').strip()
        pin_hash = generate_password_hash(pin) if pin else None
        
        create_user(staff_user, pin_hash, is_admin=False)
        return redirect(url_for('main.login'))
...
def login():
    # Load users for PIN selection (Shim returns dict)
    users = load_users() 
    mode = request.args.get('mode', 'user')
    
    if request.method == 'POST':
        logger.info(f"[Login] POST received. Form keys: {list(request.form.keys())}")
        logger.info(f"[Login] Session before: {dict(session)}")
        
        # Apply rate limiting on POST only (login attempts)
        limiter = getattr(current_app, 'limiter', None)
        if limiter:
            try:
                limiter.check()  # Check the limit
            except:
                pass  # Limiter not configured or limit not hit
        
        if 'password' in request.form:
            conf = load_config()
            # Legacy Admin Login via Config Hash
            if check_password_hash(conf.get('ADMIN_HASH', ''), request.form['password']):
                ...
                mode = 'admin'
        elif 'pin_user' in request.form:
            username = request.form['pin_user']
            ...
            if user:
                # If no password/pin hash in DB?
                if not user['password_hash']:
                    session.permanent = True
                    session['user'] = username
                    session['is_admin'] = bool(user['is_admin'])
                    logger.info(f"[Login] Success (no PIN). Session after: {dict(session)}")
                    return redirect(url_for('main.index'))
                
                if 'pin' in request.form and check_password_hash(user['password_hash'], request.form['pin']):
                    session.permanent = True
                    session['user'] = username
                    session['is_admin'] = bool(user['is_admin'])
                    logger.info(f"[Login] Success (with PIN). Session after: {dict(session)}")
                    return redirect(url_for('main.index'))
                else:
                    if 'pin' in request.form:
                         flash("Incorrect PIN")
                    return render_template('login.html', users=users, prompt_pin_for=username, mode='user')

    return render_template('login.html', users=users, mode=mode)
...
def report_issue():
    ...
    if source_url and source_url.startswith(request.host_url):
...
def scan_page_legacy():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            items = [{"name": item_name, "image": item['image_url'] if item else None, "subtext": tracking}]
        ...
        return render_template('scan.html', message=msg, color=color, items=items, mode='receive', priority_alert=is_priority)
        
    return scan_page('receive') 

@main_bp.route('/scan/<mode>', methods=['GET'])
def scan_page(mode):
    logger.info(f"[Scan] scan_page called. Mode: {mode}, Session: {dict(session)}")
    if 'user' not in session:
        logger.warning(f"[Scan] No user in session, redirecting to login")
        return redirect(url_for('main.login'))
...
def api_dashboard_stats():
    ...
    from app.services.data_manager import get_dashboard_stats, get_scan_count_today, get_analytics_stats
