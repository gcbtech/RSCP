...
from app.services.auth import load_users, create_user, delete_user, update_user_password, update_user_admin_status, BASE_DIR
...
def add_user_action():
    ...
    if not username or not password:
        ...
        return redirect(url_for('admin.admin_panel', tab='maint'))
    ...
    try:
        ...
        # Using create_user shim (which I restored in auth.py)
        # Note: create_user signature in auth.py is (username, password_hash, is_admin)
    ...
    try:
        def parse_version(v):
            ...
            if not v or v == "unknown":
                return (0, 0, 0)
            # Split by dots and convert to tuple of integers
            parts = v.strip().split('.')
        ...
        # If parsing fails, don't offer update
        return False
