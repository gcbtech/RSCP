...
def load_config() -> Dict[str, Any]:
    """Load configuration, with environment variables taking priority for sensitive values."""
    config = {}
        ...
        try:
            ...
            for row in rows:
                ...
                if existing:
                    ...
                    cur.execute('''
                        UPDATE packages SET 
                        item_name=?, date_expected=?, quantity=?, image_url=?, status=?
                        WHERE id=?
                    ''', (item_name, date_final, qty, img, status, existing['id']))
                    ...
                    cur.execute('''
                        INSERT INTO packages (tracking_number, item_name, date_expected, quantity, image_url, status)
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (tracking, item_name, date_str, qty, img, status))
...
def get_scan_count_today() -> int:
    conn = get_db_connection()
    count = conn.execute("SELECT count(*) as c FROM history WHERE action='received' AND date(timestamp, 'localtime') = date('now', 'localtime')").fetchone()['c']
    conn.close()
    return count
